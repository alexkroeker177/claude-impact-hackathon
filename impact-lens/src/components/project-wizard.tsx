  ...
  return (
    <main className="min-h-screen bg-[#f5f4ee] px-5 py-8 text-slate-950 sm:px-8 sm:py-12">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-12 max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-700">New impact assessment</p><h1 className="mt-3 text-4xl font-semibold tracking-[-0.04em] sm:text-6xl">Upload once. Review the logic. Keep the evidence.</h1><p className="mt-5 text-lg leading-8 text-slate-600">ImpactLens profiles every table, makes one bounded interpretation request, then calculates only the KPIs you accept.</p><p className="mt-3 text-sm leading-6 text-slate-500">Claude receives project context, compact structural profiles, aggregate ranges, and a few redacted representative values—not your raw files or complete source rows.</p></div>
        ...
        {stage === "details" || stage === "uploading" || stage === "interpreting" ? <form className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 sm:p-8" onSubmit={beginReview}>
          <div className="grid gap-5 sm:grid-cols-2"><Field label="Project name" value={projectName} onChange={setProjectName} placeholder="e.g. Community skills pilot" required /><Field label="What is this project trying to achieve?" value={goal} onChange={setGoal} placeholder="e.g. Understand where participant outcomes improve" required /></div>
          <div className="mt-5"><Field label="Anything the analysis should pay attention to? (optional)" value={attention} onChange={setAttention} placeholder="e.g. equitable reach across locations" /></div>
