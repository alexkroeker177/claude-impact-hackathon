...
import { listProjects, type ProjectRecord } from "@/lib/db/projects";
    ...
    <main className="min-h-screen bg-[#f5f4ee] px-5 py-10 text-slate-950 sm:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-10 space-y-3">
          <ProjectLink detail="Synthetic fallback · Ready without Claude" id="demo" name="Harbor Health Collective" status="ready" />
          {projects.filter((project) => project.id !== "demo").map((project) => <ProjectLink detail={projectDetail(project)} id={project.id} key={project.id} name={project.name} status={project.status} />)}
        </div>
      </div>
    </main>
  );
...
function ProjectLink({ id, name, detail, status }: { id: string; name: string; detail: string; status: ProjectRecord["status"] | "ready" }) {
  ...
  return (
    <Link className="flex flex-col justify-between gap-4 rounded-3xl border border-slate-200 bg-white p-6 transition hover:-translate-y-0.5 hover:shadow-lg sm:flex-row sm:items-center" href={status === "review" ? `/projects/new?project=${encodeURIComponent(id)}` : `/projects/${id}`}>
      <div><h2 className="text-xl font-semibold">{name}</h2><p className="mt-2 text-sm text-slate-500">{detail}</p></div>
